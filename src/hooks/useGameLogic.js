...
export const useGameLogic = (state, setState) => {
  useEffect(() => {
    ...
    if (isChampX) {
      setState(prevState => ({ ...prevState, currentPlayer: 'X', isGameEnded: true }));
    }

    if (isChampO) {
      setState(prevState => ({ ...prevState, currentPlayer: 'O', isGameEnded: true }));
    }

    if (emptyField.length === 0 && !isChampO && !isChampX) {
      setState(prevState => ({ ...prevState, isDraw: true, isGameEnded: true }));
    }
